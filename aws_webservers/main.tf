...
data "terraform_remote_state" "public_subnet" { // This is to use Outputs from Remote State
  backend = "s3"
  config = {
    bucket = "npaul19-bucket-lab4"            // Bucket from where to GET Terraform State
    key    = "lab4/networking/terraform.tfstate" // Object name in the bucket to GET Terraform State
